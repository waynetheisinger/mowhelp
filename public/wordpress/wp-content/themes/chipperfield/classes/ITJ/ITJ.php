...
class ITJ {
    ...
    public static function load_scripts() {
        ...
        self::load_localize_script('main', array('jquery'));

        // wp_enqueue_script(
        //         ITJ_CLASSES_NAMESPACE . '-vue',
        //         "https://cdn.jsdelivr.net/npm/vue",
        //         array('jquery')
        // );
        // wp_enqueue_script(
        //         ITJ_CLASSES_NAMESPACE . '-vue-axios',
        //         "https://cdn.jsdelivr.net/npm/axios/dist/axios.min.js",
        //         array(ITJ_CLASSES_NAMESPACE . '-vue')
        // );
    ...
    public static function load_localize_script($script_name, $dependancies = array(), $object_name = '', $data = '') {
        ...
        if (!file_exists(ITJ_ASSETS_SCRIPT_PATH . $script_name . '.js')) {
            return false;
        }

        wp_enqueue_script(
                ITJ_CLASSES_NAMESPACE . '-' . $script_name,
                ITJ_ASSETS_SCRIPT_URI . $script_name . '.js',
                $dependancies,
                filemtime(ITJ_ASSETS_SCRIPT_PATH . $script_name . '.js')
        );
    ...
    public static function load_style($style_name, $dependancies = array()) {
        ...
        if (!file_exists(ITJ_ASSETS_STYLE_PATH . $style_name . '.css')) {
            return false;
        }

        wp_enqueue_style(
                ITJ_CLASSES_NAMESPACE . '-' . $style_name,
                ITJ_ASSETS_STYLE_URI . $style_name . '.css',
                $dependancies,
                filemtime(ITJ_ASSETS_STYLE_PATH . $style_name . '.css')
        );
    }
    ...
    public static function load_classes($class_list = []) {
        ...
        foreach ($class_list as $class_name) {
            $file = ITJ_CLASSES_PATH . '/' . $class_name;
            if (!file_exists($file)) {
                return;
            }
    ...
    public static function load_template($plugin_template_path, $data = array()) {
        if (file_exists(ITJ_TEMPLATES_PATH . $plugin_template_path)) {
            require ITJ_TEMPLATES_PATH . $plugin_template_path;
        }
    }

    public static function get_image_url($image_path) {
        return ITJ_ASSETS_IMAGE_URI . $image_path;
    }
    ...
    public static function get_api_namespace() {
        return sanitize_title(ITJ_CLASSES_NAMESPACE) . '/' . ITJ_API_VERSION;
    }
    ...
    public static function view_project_template($template) {
        ...
        if (file_exists(get_template_directory() . get_post_meta($post->ID, '_wp_page_template', true))) {
            ...
            $file = ITJ_THEME_TEMPLATES_PATH . get_post_meta(
                            $post->ID, '_wp_page_template', true
            );
        }
