...
class JQ_Archive_List_Widget extends WP_Widget {
	public const JS_FILENAME = 'jal.js';
	public const CSS_FILENAME = 'jal.css';
	public const JS_ADMIN_FILENAME = 'jal.admin.js';
	...
	public function __construct() {
		parent::__construct( 'jal_widget', 'JS Archive List Widget', [
	...
	public function admin_enqueue_scripts( $hook ) {
		if ( 'widgets.php' === $hook ) {
			wp_enqueue_script( 'jquery-archive-list', JAL_BASE_URL . '/assets/js/'
													  . self::JS_ADMIN_FILENAME, [ 'jquery' ], JAL_VERSION, true );
		}
	}
	...
	public function expand_month( $year, $month, $cur_post_year, $cur_post_month, $expandConfig ) {
		...
		$expand_by_cur_month      = get_the_ID() >= 0 && ( $month->month == $cur_post_month )
									&& ( $year == $cur_post_year )
									&& $expand_by_cur_post;
		$expand_by_cur_date_month = ( $year == date( 'Y' ) && $month->month == date( 'n' ) )
									&& $expand_by_cur_post_date;

		return $expand_by_cur_date_month || $expand_by_cur_month || 'all' === $expandConfig;
	}
	...
	private function html_months( $year, $cur_post_year, $cur_post_month, $expand_year ) {
		$months                = $this->data_source->get_months( $year );
		$html                  = '';
		$frontend              = new JS_Archive_List_Frontend( $this->config );
		$expand_list_css_class = '';

		foreach ( $months as $month ) {
			$month_url    = get_month_link( $year, $month->month );
			$expand_month = $expand_year || $this->expand_month( $year, $month, $cur_post_year, $cur_post_month, $this->config['expand'] );
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['showpost']      = isset( $new_instance['showpost'] ) ? 1 : 0;
		$instance['showcount']     = isset( $new_instance['showcount'] ) ? 1 : 0;
		$instance['onlycategory']  = isset( $new_instance['onlycategory'] ) ? 1 : 0;
		$instance['only_sym_link'] = isset( $new_instance['only_sym_link'] ) ? 1 : 0;
		$instance['accordion']     = isset( $new_instance['accordion'] ) ? 1 : 0;
